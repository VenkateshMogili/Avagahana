...
if(isset($_SESSION['stuid']))
{
	...
	$sd=$_SESSION['stuid'];
	$cat=mysql_real_escape_string(htmlentities(htmlspecialchars_decode($_POST['category'])));
//video file uploading...
    ...
    $vpb_allowed_file_extensions = array("docx","doc","odt","ppt","pptx","pdf","txt","png","jpg","jpeg","gif");
    $date=date('d-m-Y');
    $time=time();
          ...
          if (!in_array($vpb_file_extensions, $vpb_allowed_file_extensions)) 
    {
        ...
        echo "<script>alert('only .pdf or .docx or .pptx or .doc or .odt or .txt or .png or .jpg or .jpeg or .gif formats files are allowed');window.location='index.php';</script>";
    }
    else 
    {
...
exit;
	...
	if(mysqli_num_rows($s)!=1)
	{
		...
		if($k==true)
		{
			$l=mysqli_query($con,"SELECT * FROM files where file_name='$venkateshfname' and f_type='doc'");
			if(mysqli_num_rows($l)!=1)
			{
				...
				$p=mysqli_query($con,"INSERT INTO files (file_name,f_type,id,category) values('$venkateshfname','doc','$id','$cat')");
				if($p==true)
				{

 echo "<center><div style='width:100%;height:100%;background-color:lightgray;'><h2 style='color:#3cc74e;background-color:white;padding:10px;font-family:georgia;border:2px solid green;'>Document Uploaded Successfully...</h2><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><a href='index.php' style='color:teal;font-size:20px;font-family:arial;text-decoration:none;border:2px solid red;background-color:white;padding:10px;border-radius:100px;'><--Go back to Homepage</a></div></center>";				}
				else{
